    ...
    while (ros::ok()) {
        ...
        if ((g_radius_min < WALL_FOLLOW_RADIUS) && (g_index_min_dist_ping < g_index_tangent_left)) {
 	    ...
 	    ROS_WARN("Get parallel to the wall");
            ...
            while (g_clearance_tan_test > 1.05 * g_radius_left) {
        ...
        twist_commander.publish(twist_cmd);
        while ((g_radius_tan_test <= alpha * g_radius_left) && (g_index_min_dist_ping >= g_index_tangent_left)) {
        ...
        if ((g_radius_min < WALL_FOLLOW_RADIUS) && (g_index_min_dist_ping < g_index_tangent_left)) {
            ...
            while (g_clearance_tan_test > 1.028 * g_radius_left) {
                ROS_WARN("trying to reconnect to wall w/ circular trajectory");
                ros::spinOnce();
                loop_timer.sleep();
            }
