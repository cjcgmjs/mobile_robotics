...
double g_radius_left; //radius of ping to robot's left; want to keep this about WALL_FOLLOW_RADIUS
double g_radius_min; //radius of closest ping from -pi/2 to +pi/2 (exclude pings behind robot)
double g_radius_tan_test; //radius of ping used to look ahead from robots left
double g_clearance_tan_test; //cos(theta)*radius of look-ahead ping; i.e. clearance
...
void laserCallback(const sensor_msgs::LaserScan& laser_scan) {
    ...
    if (g_index_front < 0) {
        ...
        g_index_90deg_left = (int) ((angle - g_angle_min) / g_angle_increment);
        g_index_90deg_right = (int) ((-angle - g_angle_min) / g_angle_increment);
    ...
    g_clearance_tan_test = sqrt(g_radius_left * g_radius_left + g_radius_tan_test * g_radius_tan_test - 2 * g_radius_left * g_radius_tan_test);
    ...
    ROS_INFO("angle max ping dist = %f at index %d", g_radius_left, g_index_90deg_left);
    ROS_INFO("angle min ping dist = %f at index %d", laser_scan.ranges[g_index_90deg_right], g_index_90deg_right);
    ROS_INFO("left tangent ping dist = %f at index %d", g_radius_tan_test, g_index_tangent_left);
...
int main(int argc, char **argv) {
    ...
    while (ros::ok()) {
	    ...
	    if (g_clearance_tan_test < (1.05 * g_radius_left * (sin(angle) / tan(angle - DANG_TANGENT_APPROX) - cos(angle)))) {
	 	    ROS_WARN("Get parallel to the wall");
		    twist_cmd.angular.z = -YAW_RATE;
		    twist_cmd.linear.x = -g_radius_left * YAW_RATE * sin(angle);
	    	    twist_cmd.linear.y = g_radius_left * YAW_RATE * cos(angle);
		    twist_commander.publish(twist_cmd);

		    while (g_clearance_tan_test < (1.05 * g_radius_left * (sin(angle) / tan(angle - DANG_TANGENT_APPROX) - cos(angle)))) {
		        ROS_WARN(" < trying to get parallel to wall w/ circular trajectory");
		        ros::spinOnce();
		        loop_timer.sleep();
		    }
	    } else {
	 	    ROS_WARN("Get parallel to the wall");
		    twist_cmd.angular.z = YAW_RATE;
		    twist_cmd.linear.x = std::max(g_radius_left,  WALL_FOLLOW_RADIUS) * YAW_RATE * sin(angle);
	    	    twist_cmd.linear.y = -std::max(g_radius_left,  WALL_FOLLOW_RADIUS) * YAW_RATE * cos(angle);
		    twist_commander.publish(twist_cmd);

		    while (g_clearance_tan_test > (1.05 * g_radius_left * (sin(angle) / tan(angle - DANG_TANGENT_APPROX) - cos(angle)))) {
		        ROS_WARN(" > trying to get paralle to wall w/ circular trajectory");
		        ros::spinOnce();
		        loop_timer.sleep();
		    }	    
            }
            ...
            twist_commander.publish(twist_cmd);
	    ROS_WARN("Done getting parallel to the wall");
        }
        ...
        twist_commander.publish(twist_cmd);
        while ((g_radius_tan_test <= alpha * g_radius_left) && (g_radius_min > WALL_FOLLOW_RADIUS)) {
        ...
        if ((g_radius_min < WALL_FOLLOW_RADIUS)) {
            ...
            ROS_WARN("min ping dist = %f at index %d", g_radius_min, g_index_min_dist_ping);
        } else if (g_radius_tan_test > alpha * g_radius_left) {
